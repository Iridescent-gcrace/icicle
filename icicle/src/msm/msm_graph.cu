...
#define MAX_TH 256
    ...
    cudaError_t bucket_method_msm_with_cuda_graph(
      unsigned bitsize,
      unsigned c,
      const S* scalars,
      const A* points,
      ...
      cudaGraphExec_t* graph_exec)
    {
      ...
      if (h_nof_buckets_to_compute > h_nof_large_buckets) {
        ...
        accumulate_buckets_kernel<<<NUM_BLOCKS, NUM_THREADS, 0, stream>>>(
          buckets, 
          sorted_bucket_offsets + h_nof_large_buckets,
          sorted_bucket_sizes + h_nof_large_buckets,
          sorted_single_bucket_indices + h_nof_large_buckets,
          sorted_point_indices,
          d_points,
          nof_buckets + nof_bms_per_msm,
          h_nof_buckets_to_compute - h_nof_large_buckets,
          c + bm_bitsize,
          c
        );
      }
      ...
      unsigned nof_final_results_per_msm = nof_bms_per_msm;
      P* final_results;
      if (is_big_triangle || c == 1) {
        ...
        for (unsigned i = 0;; i++) {
          ...
          initialize_buckets_kernel<<<(target_buckets_count * batch_size + 255) / 256, 256>>>(
            target_buckets, target_buckets_count * batch_size);
          ...
          for (unsigned j = 0; j < target_bits_count; j++) {
            ...
            if (!is_odd_c || !is_first_iter) {
              single_stage_multi_reduction_kernel<<<NUM_BLOCKS, NUM_THREADS, 0, stream>>>(
                ...
                is_last_iter ? target_buckets : temp_buckets1,
                1 << source_bits_count,
                1 << (source_bits_count - j + (is_odd_c ? 1 : 0)),
                is_last_iter ? 1 << target_bits_count : 0,
                1 << target_bits_count,
                0,
                (1 << target_bits_count) - 1,
                nof_threads
              );
            }
            ...
            single_stage_multi_reduction_kernel<<<NUM_BLOCKS, NUM_THREADS, 0, stream_reduction>>>(
              is_first_iter ? source_buckets : temp_buckets2,
              is_last_iter ? target_buckets : temp_buckets2,
              1 << target_bits_count,
              1 << (target_bits_count - j),
              is_last_iter ? 1 << target_bits_count : 0,
              1 << (target_bits_count - (is_odd_c ? 1 : 0)),
              1,
              (1 << (target_bits_count - (is_odd_c ? 1 : 0))) - 1,
              nof_threads
            );
          }
          CHK_IF_RETURN(cudaEventRecord(event_finished_reduction, stream_reduction));
          CHK_IF_RETURN(
            cudaStreamWaitEvent(stream, event_finished_reduction));
      ...
      final_accumulation_kernel<P, S><<<NUM_BLOCKS, NUM_THREADS, 0, stream>>>(
        final_results, 
        are_results_on_device ? final_result : d_allocated_final_result,
        batch_size, 
        nof_final_results_per_msm, 
        c
      );
  ...
  cudaError_t msm(const S* scalars, const A* points, int msm_size, MSMConfig& config, P* results)
  {
    ...
    return CHK_STICKY(bucket_method_msm_with_cuda_graph(
      ...
      config.is_async, stream, config.ctx.graph, config.ctx.graph_exec));
  }
